...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.alterTable('fact_ordem_pagamento', (t) => {
    t.string('num_empenho_base', 20).nullable().after('num_empenho');
  ...
  await knex.raw(`
    ...
    SET num_empenho_base = TRIM(SUBSTRING_INDEX(num_empenho, '/', 1))
  `);
  ...
  await knex.schema.alterTable('dim_setor', (t) => {
    t.text('num_empenhos').nullable().after('palavras_chave');
  });
}
