...
import {
  ...
  Trash2, X, TrendingUp, Info, FileText,
} from 'lucide-react';
...
function ConfigModal({
  ...
  filename: string;
  token: string;
  onConfirm: (entidadeId: number, periodo: string, sistemaOrigem: string) => void;
  onCancel: () => void;
}) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white rounded-t-2xl">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-[#C9A84C]/15 rounded-xl flex items-center justify-center">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-start gap-2 bg-blue-50 rounded-xl p-3">
            ...
            <p className="text-xs text-blue-700">
              Importe a <strong>Listagem de Receita Arrecadada</strong> exportada pelo SIAFIC.
              Os dados serão extraídos por rubrica, fonte de recurso e período.
            </p>
          </div>
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t border-gray-100">
          ...
          <button
            onClick={() => canConfirm && onConfirm(parseInt(entidadeId), periodo, sistemaOrigem)}
            disabled={!canConfirm}
            className={cn(
...
export default function ImportacaoReceitaPage() {
  ...
  const { data: jobsData, refetch: refetchJobs } = useQuery({
    queryKey: ['import-receita-jobs'],
    queryFn: async () => {
      try {
        return await apiRequest<{ jobs: ImportJob[]; total: number }>(
          '/import/jobs',
          { token, params: { limit: 20, tipo: 'RECEITA' } },
        );
      } catch { /* API offline */ }
    },
    enabled: !!token,
  ...
  const handleConfirmUpload = async (entidadeId: number, periodo: string, sistemaOrigem: string) => {
    ...
    try {
      ...
      formData.append('tipo_relatorio', 'RECEITA');
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 space-y-5">
          ...
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
            ...
            <div className="text-xs text-amber-800 space-y-0.5">
              <p className="font-semibold">Formato esperado: Listagem de Receita Arrecadada (SIAFIC)</p>
              <p className="text-amber-700">Exporte o relatório em PDF pelo sistema SIAFIC e faça o upload abaixo. O sistema extrai automaticamente data, rubrica, fornecedor, fonte de recurso e valor.</p>
            </div>
          </div>
          ...
          <button
            ...
            className={cn(
              ...
              selectedFile && !uploading
                ? 'bg-[#C9A84C] hover:bg-[#b8953d] shadow-md hover:shadow-lg'
                : 'bg-gray-200 text-gray-400 cursor-not-allowed',
            )}
          >
            {uploading ? (
              ...
              <><Upload size={16} /> Importar Listagem de Receita</>
            )}
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
            <h2 className="text-base font-semibold text-navy-800">Histórico de Importações — Receita</h2>
            {jobsData?.total != null && (
          ...
          {!jobsData?.jobs?.length ? (
            <div className="p-12 text-center text-gray-400">
              ...
              <p className="text-sm">Nenhuma importação de receita realizada ainda.</p>
              <p className="text-xs mt-1 text-gray-300">Faça o upload de uma listagem de receita arrecadada do SIAFIC.</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">
                      ...
                      {(job as any).periodo && (
                        <> · <span className="text-[#C9A84C] font-medium">{(job as any).periodo}</span></>
                      )}
