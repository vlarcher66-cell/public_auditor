...
function EntidadeModal({ entidade, token, onClose, onSaved }: {
  entidade: Entidade | null; token: string; onClose: () => void; onSaved: () => void;
}) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ nome: nome.trim(), cnpj: cnpj.trim() || null, tipo, ativo }),
      });
...
export default function EntidadePage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API}/entidades`, { headers: authHeader(token) });
      if (res.ok) { const d = await res.json(); setEntidades(d.rows ?? d); }
    } catch { /* offline */ }
    setLoading(false);
  }, [token]);
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Gerencie os fundos, prefeituras e órgãos do sistema</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border overflow-hidden">
        ...
        {entidades.length > 0 && (
          <div className="px-4 py-2 border-t bg-gray-50 text-xs text-gray-400">
            {entidades.length} entidade{entidades.length !== 1 ? 's' : ''} cadastrada{entidades.length !== 1 ? 's' : ''}
          </div>
        )}
      </div>

      {modal.open && (
        <EntidadeModal entidade={modal.item} token={token}
